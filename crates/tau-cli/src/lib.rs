...
impl Harness {
    ...
    fn new(
        ...
        let agent = spawn_local_participant("agent", ClientKind::Agent, |reader, writer| {
            tau_agent::run(reader, writer).map_err(|error| error.to_string())
        })?;
        let filesystem_tool =
            spawn_local_participant("filesystem-tool", ClientKind::Tool, |reader, writer| {
                tau_ext_fs::run(reader, writer).map_err(|error| error.to_string())
            })?;
        let shell_tool =
...
///
/// Prefers `$XDG_RUNTIME_DIR/tau/daemon.sock` when available, falling
/// back to a project-local `.tau/daemon.sock`.
#[must_use]
pub fn default_socket_path() -> PathBuf {
    dirs::runtime_dir()
        .map(|dir| dir.join("tau").join("daemon.sock"))
        .unwrap_or_else(|| PathBuf::from(".tau").join("daemon.sock"))
}
