    ...
    pub fn route_tool_request(
        ...
        let route_report = bus
            .send_to(
                ...
                Some(requester_id),
                Event::ToolInvoke(shlop_proto::ToolInvoke {
...
pub enum ToolActivityOutcome {
    Requested {
        arguments: shlop_proto::CborValue,
    },
    Result {
        result: shlop_proto::CborValue,
    },
    Error {
        message: String,
        details: Option<shlop_proto::CborValue>,
    },
}
...
pub enum SessionStoreError {
    ...
    Decode {
        path: PathBuf,
        source: shlop_proto::DecodeError,
    },
    Encode {
        path: PathBuf,
        source: shlop_proto::EncodeError,
    },
}
...
mod tests {
    ...
    fn connection_abstraction_is_transport_independent_for_in_memory_clients() {
        ...
        let first_report = bus.publish(Event::ToolResult(shlop_proto::ToolResult {
            call_id: "call-1".to_owned(),
            tool_name: "demo.echo".to_owned(),
    ...
    fn deterministic_agent_and_tool_complete_one_vertical_slice() {
        ...
        let agent_thread = thread::spawn(move || {
            ...
            shlop_agent::run(agent_reader, agent_runtime_stream)
        ...
        let tool_thread = thread::spawn(move || {
            ...
            shlop_ext_fs::run(tool_reader, tool_runtime_stream)
        ...
        bus.send_to(
            &agent_id,
            Some(&ui_id),
            Event::LifecycleDisconnect(shlop_proto::LifecycleDisconnect {
